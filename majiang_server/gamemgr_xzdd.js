...
function calcSelfFan(seatData, otherSeat) {
    ...
    else if((seatData.tinged || seatData.gangTinged) && (otherSeat.tinged || otherSeat.gangTinged)) {
        selfFan = 2;
    }
    else{
...
function calcScoreByBaibalzhungSanwanpaiGang(game, seatData, gangPais){
    ...
    if(gangPais.indexOf(seatData.holds[seatData.holds.length - 1].toString()) != -1){
...
function checkCanHuForPeng4(seatData, targetPai, arrayPengForPiaoHu, isCanGangTing, isExistingFeng, isCheckInHu) {
    ...
    // if(isCheckInHu){
    //     if(!seatData.piaoTinged && seatData.canPiaoTing){
    //         flag = false;
    //     }
    // }
...
function checkYise(seatData, isExistingPing, isExistingTiao, isExistingWan, isExistingOneOrNine, isExistingFeng) {
    let flag= false;
...
function sendOperations(game,seatData,pai) {
    if(hasOperations(seatData) || (seatData.paisAvailableTing && seatData.paisAvailableTing.length > 0) ||  isTinged(seatData)){
        ...
        if(seatData.canHu){
            if(!seatData.piaoTinged && calcPengCount(seatData) == 4){
                seatData.canHu = false;
            }
        }
...
function doUserMoPai(game){
    ...
    if(isTinged(turnSeat) || turnSeat.hued){
        ...
        if(turnSeat.canHu && calcPengCount(turnSeat) == 4){
            seatData.canHu = false;
        }
...
exports.chuPai = function(userId,pai){
    ...
    for(let i = 0; i < game.gameSeats.length; ++i) {
        ...
        if (!ddd.canHu) {
            ...
            if(!ddd.piaoTinged && calcPengCount(ddd) == 4) {
                ddd.canHu = false;
                //piaoting을 해야 후룰 할수 있다.
            }
            else{
