...
function checkCanMingGang(game,seatData,targetPai){
    ...
    if(seatData.tinged && game.conf.tinghoefeigang){
        return;
    }
    // if(getMJType(targetPai) == seatData.que){
    //     return;
...
function checkCanHu(game,seatData,targetPai) {
    ...
    // 손에 13개의 패가 다 있으면 팅을 해야 후할수 있다.
    if(seatData.holds.length == 14 && !seatData.tinged){
        return;
    }
                    ...
                    if (seatData.availableTingMap[kk].hasOwnProperty(k)) {
                        ...
                        if(targetPai == k && targetPai == kk){
                            ...
                            if(seatData.game.conf.renshu == 3 && checkParams.isExistingOneOrNine){
                                if (checkCanHuForPeng4(seatData, targetPai, data.arrayPengForPiaoHu)) {
                                    seatData.canHu = true;
                                }
                                else{
                                    seatData.canHu = false;
                                ...
                                else if (game.conf.yise && checkYise(checkParams.isExistingPing, checkParams.isExistingTiao, checkParams.isExistingWan, checkParams.isExistingOneOrNine)) {
    ...
    for(let k in seatData.tingMap){
        if(seatData.tingMap.hasOwnProperty(k)){
            if(targetPai == k){
                ...
                if(seatData.game.conf.renshu == 3 && checkParams.isExistingOneOrNine){
                    if (checkCanHuForPeng4(seatData, targetPai, seatData.tingMap[k].arrayPengForPiaoHu)) {
                        seatData.canHu = true;
                    }
                    else{
                        seatData.canHu = false;
...
function checkTingCondition(seatData){
    ...
    if(!isExistingOneOrNine){
        for(let p of seatData.holds){
            if((parseInt(p) >= seatData.game.DongStartID) && (parseInt(p) < seatData.game.DongStartID + 4)){
                if(seatData.countMap[p] == 2 || seatData.countMap[p] == 1){
...
function calcScoreByBaibalzhungSanwanpaiGang(game, seatData){
    let fen = game.conf.baseScore;
    //깡을 한 사람이 짱일때 점수를 2배로 해주고 나머지 사람들에게 2배씩 덜어준다.
    if(game.button == seatData.seatIndex){
        seatData.score += game.basic_score * fen * 2 * (game.seatCount - 1);
        seatData.levelScore += game.basic_score * fen * 2 * (game.seatCount - 1);
        for(let seat of game.gameSeats){
            if(seat.seatIndex != seatData.seatIndex){
                seat.score -= game.basic_score * fen * 2;
                seat.levelScore -= game.basic_score * fen * 2;
            }
        }
    }
    else{
        seatData.score += game.basic_score * fen * game.seatCount;
        seatData.levelScore += game.basic_score * fen * game.seatCount;
        for(let seat of game.gameSeats){
            if(seat.seatIndex == seatData.seatIndex){
                continue;
            }
            if(game.button == seat.seatIndex) {
                ...
                seat.score -= game.basic_score * fen;
                seat.levelScore -= game.basic_score * fen;
            }
        }
    }
    let resultScores = [];
...
function checkYise(isExistingPing, isExistingTiao, isExistingWan, isExistingOneOrNine) {
    let flag= false;
    if(isExistingOneOrNine){
...
function checkCanTingOrHuCondition(seatData) {
    ...
    for(let k of existingPais) {
        ...
        if(Object.keys(seatData.tingMap).length > 0 && seatData.tingMap.constructor === Object){
            let checkParams = checkTingCondition(seatData, k);
            ...
            if(seatData.game.conf.renshu == 3){
                ...
                else if (seatData.game.conf.yise && checkYise(checkParams.isExistingPing, checkParams.isExistingTiao, checkParams.isExistingWan, checkParams.isExistingOneOrNine)) {
                    availableCondition = true;
                }
            }

            if(availableCondition){
                seatData.paisAvailableTing.push(k);
...
function checkCanTingPai(game,seatData){
    ...
    // // 검사를 위하여 동서남북, 삼원패를 뽑는다.
    // let tempPais = [];
    // for(let pai of seatData.holds){
    //     if(parseInt(pai) < 27){
    //         continue;
    //     }
    //     tempPais.push(pai);
    // }
    //
    // seatData.holds.sort(compareFn);
    //
    // let startIndex = 0;
    // let n = 0;
    // for(let pai of seatData.holds){
    //     if(parseInt(pai) >= seatData.game.DongStartID){
    //         startIndex = n;
    //         break;
    //     }
    //     n++;
    // }
    // seatData.holds.splice(n, tempPais.length);
    // for(let p of tempPais){
    //     seatData.countMap[p]--;
    // }
    ...
    /// 뽑아놓은 동서남북, 삼원패가 팅할때 부합되는가를 판정해야 한다.
    /// 그래서 동 3개 등, 동 2개 등이 있는가를 판정하고 팅할때 어느패가 필요한가를 얻어야 한다.
    /// 먼저 동서남북 삼원패를 가지고 MAP 를 만든다.
    // let tempMap = {};
    // for(let p of tempPais){
    //     if(!tempMap[p]){
    //         tempMap[p] = 1;
    ...
    //     if(tempMap[k] == 2 || tempMap[k] == 1){
    //         resultPais.push(k);
    //     }
    // }
    //
    // // 동서남북, 삼원패를 뽑은 패들을 가지고 얻은 tingMap와
    // // 동서남북, 삼원패를 가지고 팅을 할수 있는 패들을 얻은 resultPais의 개수를 가지고
    // // 진짜 팅을 할수 있는지를 판정해야 한다.
    // // 그것은 tingMap의 개수가 2개가 되여야 한다는것이다.
    // let tingMapKeys = [];
    // for(let k in seatData.tingMap){
    //     tingMapKeys.push(k);
    // }
    //
    // // let canTingFlag = false;
    // // let totlaCount = tingMapKeys.length + resultPais.length;
    // // if(totlaCount == 1 || totlaCount == 2){
    // //     canTingFlag = true;
    // //     for(let pp of resultPais){
    // //         seatData.tingMap[pp] = {
    // //             pattern:"normal",
    // //             fan:0
    // //         }
...
function hasOperations(seatData){
    ...
    seatData.canTing || seatData.canGangTing);
...
function sendOperations(game,seatData,pai) {
    if(hasOperations(seatData) || (seatData.paisAvailableTing && seatData.paisAvailableTing.length > 0) ||  seatData.tinged){
        if(pai == -1){
...
exports.setReady = function(userId,callback){
    ...
    if(game == null){
        ...
        for(let i = 0; i < game.seatCount; ++i){
            ...
            if(sd.tingPaiClicked || (sd.tinged && !sd.tingPaiClicked) == false){
                sd.paisAvailableTing = [];
            }
    ...
    userMgr.broacastInRoom('user_ready_push',{userid:userId,ready:true, score:seat.score},userId,true);
};
...
exports.begin = function(roomId) {
    ...
    for(let i = 0; i < seats.length; ++i){
        ...
        //通知游戏开始
        userMgr.sendMsg(s.userId,'game_begin_push',{button:game.button, dicePlayed: dicePlayed[roomId], levelScore: LevelScore[roomId]});
        // if(!dicePlayed){
...
exports.dice_start = function(userId){
    ...
    // if(Object.keys(gameSeatsOfUsers).length == 0){
    //     return;
    // }
    ...
    if(diceUserList[roomId].length == roomInfo.seats.length){
        ...
        for(let seat of roomInfo.seats){
            userMgr.broacastInRoom('dice_play_push',{dicePlayed: false, numList1: diceNumberList1[roomId], numList2: diceNumberList2[roomId]},seat.userId,true);
        }
        dicePlayed[roomId] = true;
    }

    // if(diceNumberList1.length == 0 || diceNumberList2.length == 0){
    //     let n;
    //     for(let i = 0; i < 12; i++){
    //         n = Math.floor(Math.random() * 6) + 1;
    //         diceNumberList1.push(n);
    //     }
    //
    //     for(let i = 0; i < 12; i++){
    //         n = Math.floor(Math.random() * 6) + 1;
    //         diceNumberList2.push(n);
    //     }
    // }
    ...
    // let roomId = roomMgr.getUserRoom(userId);
    // let roomInfo = roomMgr.getRoom(roomId);
    // let playerCount = roomInfo.conf.playerCount;
    //
    // if(diceUserList.length == playerCount){
    //     userMgr.sendMsg(userId,'dice_play_push', {dicePlayed: true});
    //     return;
    // }
    //
    // if(diceUserList.indexOf(userId) != -1){
    //     diceUserList.push(userId);
    // }
    //
    // if(diceNumberList1.length == 0 || diceNumberList2.length == 0){
    //     let n;
    ...
    // if(diceUserList.length == playerCount){
    //     for(let seat of roomInfo.seats){
    //         userMgr.sendMsg(seat.userId,'dice_play_push', {dicePlayed: false, numList1: diceNumberList1, numList2: diceNumberList2});
    //     }
    // }
...
exports.chuPai = function(userId,pai){
    ...
    if(seatData.tinged && seatData.tingMap != null && Object.keys(seatData.tingMap).length == 0){
        // let n = 0;
        // for(let p of seatData.holds){
        //     if(parseInt(p) == parseInt(seatData.tingPai)){
        //         seatData.holds.splice(n, 1);
        //         break;
        //     }
        //     n++;
        // }
        // seatData.tingMap[seatData.tingPai]--;
...
exports.gang_ting = function(userId, data) {
    ...
    calcScoreByBaibalzhungSanwanpaiGang(seatData.game, seatData);
    ...
    if(resultPais[0] == resultPais[1] && resultPais[2] == resultPais[1] && resultPais[2] == resultPais[3]){
...
exports.gang_baibalzung = function(userId, data) {
    ...
    calcScoreByBaibalzhungSanwanpaiGang(game, seatData);
...
function doGang(game,turnSeat,seatData,gangtype,numOfCnt,pai){
    ...
    // 깡을 한 user가 짱인 경우
    let fen = game.conf.baseScore;
    if(parseInt(seatData.gangPai[seatData.gangPai.length - 1]) >= parseInt(game.DongID)){
        fen *= 2;
    }
    if(game.button == seatData.seatIndex){
        if(gangtype == 'angang'){
            for(let seat of game.gameSeats){
                if(game.button == seat.seatIndex){
                    seat.score += game.basic_score * fen * 2 * (game.seatCount - 1);
                    seat.levelScore += game.basic_score * fen * 2 * (game.seatCount - 1);
                }
                else{
                    seat.score -= game.basic_score * fen * 2;
                    seat.levelScore -= game.basic_score * fen * 2;
            ...
            seatData.score += game.basic_score * fen * 2;
            seatData.levelScore += game.basic_score * fen * 2;

            let otherSeat = game.gameSeats[seatIndexToOutPengPai];
            otherSeat.score -= game.basic_score * fen * 2;
            otherSeat.levelScore -= game.basic_score * fen * 2;
        }
        else{
            seatData.score += game.basic_score * fen * 2;
            seatData.levelScore += game.basic_score * fen * 2;

            turnSeat.score -= game.basic_score * fen * 2;
            turnSeat.levelScore -= game.basic_score * fen * 2;
        }
    }
    else{
        // 깡을 한 user가 짱이 아닌 경우
        if(gangtype == 'angang'){
            for(let seat of game.gameSeats){
                ...
                else if(seatData.seatIndex == seat.seatIndex){
                    //깡한 사람인 경우
                    seat.score += game.basic_score * fen * game.seatCount;
                    seat.levelScore += game.basic_score * fen * game.seatCount;
                }
                else{
                    seat.score -= game.basic_score * fen;
                    seat.levelScore -= game.basic_score * fen;
            ...
            let otherSeat = game.gameSeats[seatIndexToOutPengPai];
            if(game.button == otherSeat.seatIndex){
                // 패를 낸 사람이 짱인 경우
                otherSeat.score -= game.basic_score * fen * 2;
                otherSeat.levelScore -= game.basic_score * fen * 2;
                ...
                // 패를 낸 사람이 짱이 아닌 경우
                otherSeat.score -= game.basic_score * fen;
                otherSeat.levelScore -= game.basic_score * fen;

                seatData.score += game.basic_score * fen;
                seatData.levelScore += game.basic_score * fen;
            }
        }

        else{
            if(game.button == turnSeat.seatIndex){
                // 패를 낸 사람이 짱인 경우
                turnSeat.score -= game.basic_score * fen * 2;
                turnSeat.levelScore -= game.basic_score * fen * 2;
                ...
                // 패를 낸 사람이 짱이 아닌 경우
                turnSeat.score -= game.basic_score * fen;
                turnSeat.levelScore -= game.basic_score * fen;
...
exports.hu = function(userId){
    ...
    //점수계산
    let baseScore = game.basic_score * game.conf.baseScore;
    let fan = 1;
    if(game.huedByQidui){
        if(game.conf.qidui4){
            fan *= 4;
    ...
    else if(game.conf.piaohu && game.huedByPiaohu){
        fan *= 2;
    }
    if(game.conf.yise && game.huedByHunyise){
        fan *= 2;
    }
    else if(game.conf.yise && game.huedByQingyise){
        fan *= 4
    }
    ...
    if(isZimo) {
        ...
        if (seatData.seatIndex == game.button) {
            for (let seat of game.gameSeats) {
                //짱인 사람
                if (seat.seatIndex == seatData.seatIndex) {
                    seat.score += baseScore * fan * 2 * (game.seatCount - 1);
                    seat.levelScore += baseScore * fan * 2 * (game.seatCount - 1);
                }
                else {
                    //짱이 아닌 사람
                    seat.score -= baseScore * fan * 2;
                    seat.levelScore -= baseScore * fan * 2;
                }
                // 깡팅이거나 팅일때 2배이므로
                if(seat.tinged || seat.gangTinged){
                    seatData.score += baseScore * fan * 2;
                    seatData.levelScore += baseScore * fan * 2;
            ...
            for (let seat of game.gameSeats) {
                ...
                else if (seat.seatIndex == seatData.seatIndex) {
                    seat.score += baseScore * fan * game.seatCount;
                    seat.levelScore += baseScore * fan * game.seatCount;
                }
                else {
                    //짱이 아닌 사람
                    seat.score -= baseScore * fan;
                    seat.levelScore -= baseScore * fan;
                }
                // 깡팅이거나 팅일때 2배이므로
                //짱에 대하여
                if (seat.seatIndex == game.button) {
                    ...
                    if (seat.tinged || seat.gangTinged) {
                        seatData.score += baseScore * fan;
                        seatData.levelScore += baseScore * fan;
        ...
        let tingFen = 1;
        if (seatData.seatIndex == game.button) {
            if (game.gameSeats[game.turn].gangTinged || game.gameSeats[game.turn].tinged) {
                tingFen = 4;
            }
            else{
                tingFen = 2;
            ...
            //패를 낸 사람이 짱이면
            if (game.gameSeats[game.turn].seatIndex == game.button) {
                if (game.gameSeats[game.turn].gangTinged || game.gameSeats[game.turn].tinged) {
                    tingFen = 4;
                }
                else{
        ...
        seatData.score += baseScore * fan * tingFen;
        seatData.levelScore += baseScore * fan * tingFen;

        game.gameSeats[game.turn].score -= baseScore * fan * tingFen;
        game.gameSeats[game.turn].levelScore -= baseScore * fan * tingFen;
    }
    ...
    //     ddd.canShunZi = false;
    //     ddd.canChuPai = false;
    //     ddd.canGangTongnansebei = false;
    //     ddd.canGangBaiBalZung = false;
    ...
    for(let i = dissolvingList.length - 1; i >= 0; --i){
        ...
        if(roomInfo != null && roomInfo.dr != null){
            if(Date.now() > roomInfo.dr.endTime){
                Logger.error(`delete room and games.`, roomId);
